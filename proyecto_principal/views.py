...
def _contexto_administrador():
    ...
    ventas_hoy = Venta.objects.filter(fecha_venta__date=hoy, estado='pagado').aggregate(total=Sum('total'), cantidad=Count('id'))
    ventas_mes = Venta.objects.filter(fecha_venta__date__gte=inicio_mes, estado='pagado').aggregate(total=Sum('total'), cantidad=Count('id'))
    pedidos_activos = Pedido.objects.exclude(estado__in=['entregado', 'cancelado']).count()
    mesas_ocupadas = Mesa.objects.filter(pedido__estado__in=['pendiente', 'preparando', 'listo']).distinct().count()
    total_mesas = Mesa.objects.count()
    productos_bajo_stock = Producto.objects.filter(disponible=True, stock__lt=10).order_by('stock')[:5]

    estados_pedidos = Pedido.objects.values('estado').annotate(cantidad=Count('id')).order_by('estado')
    estados_labels  = [e['estado'].capitalize() for e in estados_pedidos]
    estados_data    = [e['cantidad'] for e in estados_pedidos]
    estados_colores = {'pendiente': '#f6c23e', 'preparando': '#36b9cc', 'listo': '#1cc88a', 'entregado': '#4e73df', 'cancelado': '#e74a3b'}
    estados_bg = [estados_colores.get(e['estado'], '#858796') for e in estados_pedidos]

    ingresos_semana, labels_semana = [], []
    for i in range(6, -1, -1):
        dia = hoy - timedelta(days=i)
        total_dia = Venta.objects.filter(fecha_venta__date=dia, estado='pagado').aggregate(total=Sum('total'))['total'] or Decimal('0')
        ingresos_semana.append(float(total_dia))
        labels_semana.append(dia.strftime('%d/%m'))

    metodos = Venta.objects.filter(estado='pagado').values('metodo_pago').annotate(total=Sum('total'), cantidad=Count('id')).order_by('-total')
    ...
    ultimos_pedidos = Pedido.objects.select_related('mesa').order_by('-fecha_creacion')[:8]
...
def _contexto_cajera():
    ...
    metodos_data    = [float(m['total'] or 0) for m in metodos]
    metodos_colores = ['#4e73df', '#1cc88a', '#36b9cc', '#f6c23e']
    ultimas_ventas = Venta.objects.select_related('pedido').order_by('-fecha_venta')[:8]
...
def _contexto_mesero():
    ...
    pedidos_recientes = Pedido.objects.exclude(estado__in=['entregado', 'cancelado']).order_by('-fecha_creacion')[:8]
